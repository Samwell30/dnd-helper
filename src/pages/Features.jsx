...
const Features = () => {
    ...
    useEffect(() => {
        async function fetchFeatures() {
            ...
            setError(null);
            try {
                const { data } = await axios.get('https://www.dnd5eapi.co/api/2014/features');
    ...
    return (
        <div className="section">
            <h1>Features</h1>
            <p>Browse and search D&D 5e class features. Click a feature for details.</p>
                    ...
                    {uniqueFeatures.map((feature, idx) => (
                        <React.Fragment key={feature.index}>
                            <li
                                ...
                                style={{ backgroundColor: idx % 2 === 0 ? '#fff' : 'rgba(230,110,83,0.13)' }}
                            >
                                {feature.name}
                            </li>
                            {selectedFeature === feature.index && featureDetail && (
                                <li style={{ background: '#f9f9f9', padding: '8px 16px', borderLeft: '4px solid #bdbdbd' }}>
                                    <div className="equipment-detail">
                                        ...
                                        <ul>
                                            {featureDetail.desc && featureDetail.desc.map((desc, i) => (
                                                <li key={i}>{desc}</li>
                                            ))}
                                        </ul>
                                    </div>
